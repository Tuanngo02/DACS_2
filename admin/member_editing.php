            ...
            if (isset($_GET['action']) && ($_GET['action'] == 'add' || $_GET['action'] == 'edit')) {
                if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['link']) && !empty($_POST['link'])) {
                    ...
                    if (!isset($error)) {
                        if ($_GET['action'] == 'edit' && !empty($_GET['id'])) { //Cập nhật lại danh mục                            
                            $result = mysqli_query($con, "UPDATE `Users` SET `name` = '".$_POST['name']."', `link` = '".$_POST['link']."',`last_updated` = '".time()."' WHERE `Users`.`id` = ".$_GET['id'].";");
                        } else { //Thêm danh mục
                            $result = mysqli_query($con, "INSERT INTO `Users` (`id`,`name`, `link`,`created_time`, `last_updated`) VALUES (NULL,'".$_POST['name']."', '".$_POST['link']."',".time().", ".time().");");
                        }
                ...
                <form id="editing-form" method="POST" action="<?= (!empty($currentMenu) && !isset($_GET['task'])) ? "?action=edit&id=" . $_GET['id'] : "?action=add" ?>"  enctype="multipart/form-data">
                    ...
                    <div class="wrap-field">
                        ...
                        <input type="text" name="name" value="<?= (!empty($currentMenu) ? $currentMenu['username'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
                        <label>Mật khẩu: </label>
                        <input type="text" name="link" value="<?= (!empty($currentMenu) ? $currentMenu['password'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
                        <label>Tên đầy đủ: </label>
                        <input type="text" name="link" value="<?= (!empty($currentMenu) ? $currentMenu['fullname'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
                        <label>Email: </label>
                        <input type="text" name="link" value="<?= (!empty($currentMenu) ? $currentMenu['email'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
                        <label>SĐT: </label>
                        <input type="text" name="link" value="<?= (!empty($currentMenu) ? $currentMenu['phone'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
                        <label>Địa chỉ: </label>
                        <input type="text" name="link" value="<?= (!empty($currentMenu) ? $currentMenu['address'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                    <div class="wrap-field">
                        <label>Ngày sinh: </label>
                        <input type="text" name="link" value="<?= (!empty($currentMenu) ? $currentMenu['birthday'] : "") ?>" />
                        <div class="clear-both"></div>
                    </div>
                </form>
                <div class="clear-both"></div>
