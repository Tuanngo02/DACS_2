...
if (!empty($_SESSION['current_user'])) {
    ...
    if(!empty($where)){
        $products = mysqli_query($con, "SELECT * FROM `Products` where (".$where.") ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }else{
        $products = mysqli_query($con, "SELECT * FROM `Products` ORDER BY `id` DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
    }
    mysqli_close($con);
    ?>
